<template>
  <div class="app-container">
      <el-form inline>
          ...
          <el-form-item>
              <el-button size="medium"
                         type="primary"
                         icon="el-icon-plus"
                         v-if="hasPermission('BTN_AD_CARD_ADD')"
                         @click="showEditDialog('add')">新增</el-button>
              <el-button size="medium"
                         type="danger"
                         icon="el-icon-delete"
                         v-if="hasPermission('BTN_AD_CARD_DELETE')"
                         @click="toDeleteAll()">批量删除</el-button>
          </el-form-item>
      </el-form>
      <bg-set-list  dialogType="7" :countryList="countryList" :countryCode="searchParam.countryCode"></bg-set-list>
              ...
              <el-table-column label="排序"
                  ...
                  <template slot-scope="scope">
                      ...
                      <el-button
                          type="primary"
                          plain
                          v-if="hasPermission('BTN_AD_CARD_SORT')"
              ...
              <el-table-column fixed="right"
                  ...
                  <template slot-scope="scope">
                      <el-button type="text" size="mini"
                                 v-if="hasPermission('BTN_AD_CARD_EDIT')"
                                 @click="showEditDialog('edit', scope.row)">编辑</el-button>
                      <el-button size="mini" type="text"
                                 v-if="hasPermission('BTN_AD_CARD_COPY')"
                                 @click="showEditDialog('copy', scope.row)">复制</el-button>
                      <el-button size="mini" type="text"
                                 v-if="hasPermission('BTN_AD_CARD_PRODUCT')"
      ...
      <edit-dialog :dialogVisible="dialog.show" dialogTitle="广告Card" :cardType="searchParam.cardType" :urlType="urlType" :appPageEnum="appPageEnum"  :countryList="countryList" :propParam="dialog.param" @close="closeEditDialog" @confirm="confirmEditItem"></edit-dialog>
  </div>
</template>
<script>
    import { ListBoxcard, BoxcardSortChange, BatchDeleteBoxcard } from '@/request/promotion.js'
    ...
    export default {
        ...
        data () {
            return {
                searchParam: {
                    countryCode: 'MY',
                    cardType: 7, // 类型(5->营销card(跟随上腰封),7->广告card(跟随中腰封))
                    page: {
                        pageNum: 1,
                        pageSize: 10,
                        pagingSwitch: true
