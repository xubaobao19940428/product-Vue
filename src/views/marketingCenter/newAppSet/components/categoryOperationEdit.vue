    ...
    <el-dialog
        ...
        <div style="height: 100%; width: 100%">
            <el-form :rules="adDialogRules"
                ...
                <el-form-item label="关联一级类目：" prop="cateId">
                        <el-select v-model="param.cateId" filterable clearable size="medium" >
                            <el-option v-for="(item, key) in categoryListLevelOne" :key="key" :label="item.cateName" :value="(item.cateId).toString()"></el-option>
                        </el-select>
                </el-form-item>
                ...
                <el-form-item label="类目Banner" prop="cover">
...
<script>
    ...
    import { insertCategoryBanner, updateCategoryBanner } from '@/request/promotion.js'
        ...
        data () {
            return {
                ...
                adDialogRules: {
                    ...
                    cateId: [{ required: true, message: '不能为空', trigger: 'change' }],
            ...
            conformAddItem () {
                this.$refs['addDialog'].validate((valid) => {
                    if (valid) {
                        ...
                        let fetchUrl = insertCategoryBanner
                        if (this.param.id) {
                            fetchUrl = updateCategoryBanner
                        }
                        fetchUrl({ categoryBanner: this.filterData(this.param) }).then((res) => {
                            this.loading = false
