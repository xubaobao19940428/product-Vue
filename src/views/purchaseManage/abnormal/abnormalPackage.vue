<template>
  <div class="app-container">
    <el-form ref="abnormalFilterForm" inline :model="queryData">
      ...
      <span v-show="drop">
        <el-form-item label="异常类型：" class="filter-item" prop="abnormalReason">
          <el-select v-model="queryData.abnormalReason" clearable size="medium">
            <el-option
              v-for="item in packageAbnormalTypeList"
              :key="item.type"
              :label="item.remark"
              :value="item.type"
            ></el-option>
      ...
      <el-form-item class="filter-item" style="margin-left: 20px;">
        <el-button
          icon="el-icon-search"
          type="primary"
          @click="getAbnormalLogiList(1)"
          size="medium"
        >搜索</el-button>
        <el-button icon="el-icon-refresh-left" @click="resetSearch" size="medium">重置</el-button>
        <el-button
          v-if="hasPermission('BTN_PURCHASE_EXPORT_ABNORMAL_PACKAGE')"
    ...
    <div class="main-content-wrapper">
      <el-table :data="packageDataList" border height="100%">
        <el-table-column
          ...
          label="异常编号"
        ></el-table-column>
        <el-table-column prop="logiCode" label="物流单号" align="center" min-width="200px"></el-table-column>
        <el-table-column label="异常类型" prop="abnormalReason" align="center" min-width="150px"></el-table-column>
        ...
        <!--                <el-table-column prop="acceptTime" align="center" min-width="180" label="采购处理时间">-->
        <!--                    <template slot-scope="scope">-->
        <!--                        {{ scope.row.acceptTime | secondTimeFormat }}-->
        <!--                    </template>-->
        <!--                </el-table-column>-->
        <el-table-column align="center" min-width="200" label="最新处理备注">
          <template slot-scope="scope">
        ...
        <el-table-column prop="userName" align="center" label="类目分组" width="140px">
          <template slot-scope="scope">{{scope.row.purGroupName}}</template>
        </el-table-column>
        <el-table-column prop="userName" align="center" min-width="150px" label="创建人">
          <template slot-scope="scope">{{ scope.row.userName || '-' }}</template>
        </el-table-column>
        <el-table-column prop="dealUserName" align="center" min-width="150px" label="处理人">
          <template slot-scope="scope">{{ scope.row.dealUserName || '-' }}</template>
        </el-table-column>
          ...
          <template slot-scope="scope">
            <div class="order-btn-box">
              <el-button
                ...
                @click="showDealDialog(scope.row, 'edit')"
              >处理</el-button>
              <el-button
                v-if="scope.row.abnormalStatus == 8"
                type="text"
                size="mini"
                v-preventReClick="500"
                @click="showDealDialog(scope.row, 'detail')"
              >查看详情</el-button>
            </div>
          </template>
        </el-table-column>
    ...
    ></image-preview>
    <deal-package-dialog
      :propShowDialog="dialog.show"
      :dealType="dialog.type"
      :propParam="dialog.param"
      @close="dialog.show = false"
      @confirm="confirmDealPackage"
    ></deal-package-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    exportAbnormal() {
      ...
      ExportAbnormalLogi(this.filterData(params))
        .then(res => {
          if (res.ret.errcode === 1) {
    ...
    getAbnormalLogiList(page) {
      ...
      params.startTime = params.timeList ? params.timeList[0] : null;
      params.endTime = params.timeList ? params.timeList[1] : null;
      abnormalLogiPage(this.filterData(params))
        .then(res => {
          if (res.ret.errcode === 1) {
